...
const combinationSum = (candidates, target) => {
  ...
  const dfs = (path, startIndex, currentTarget) => {
    if (currentTarget === 0) {
      result.push(path)
      return null
    }
    ...
    for (let i = startIndex; i < candidates.length; i++) {
      if (currentTarget - candidates[i] >= 0) {
        dfs(path.concat(candidates[i]), i, currentTarget - candidates[i])
      }
    }
  }
...
assert.deepEqual(combinationSum(candidates, target), expected)
